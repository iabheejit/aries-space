...
**Result**: `sentinel2-ndvi-summary` on a real 3,346,740-byte crop produced DRF 6314.6×, `recommendation: "simulated_edge"`, `break_even_downlink_inr_per_gb: 0.0` (edge is cheaper than ground at any positive downlink price under the stated assumptions) — a genuine, non-degenerate, formula-traceable recommendation. See `EVIDENCE.md` for full run details. The original AC5/AC6 satnogs-proxy acceptance criteria are unchanged and still hold for that workload.

**TRL 5 claim, stated precisely (per Meera's Milestone 4 strategic audit)**: this validates the *mechanism* — a real satellite/imagery input flowing through checksum-verified ingestion into a working, formula-traceable placement recommendation — at TRL 5 ("validated prototype stage"). It does **not** by itself validate the specific numeric result at scale: this is a single scene, single AOI, single run (not a repeated-sample distribution like the SatNOGS workload's 1000-iteration median); `edge-sim` is a simulated model, not measured hardware; and the headline ₹0.00/GB break-even is substantially driven by one cost-model assumption (edge pays no per-hour compute rent, ground does). The grant application should state TRL 5 as "validated in a real-data, single-scenario benchmark, with simulated-edge and single-AOI limitations disclosed" — not as an unqualified claim. See `EVIDENCE.md`'s "Pending Evidence" for what would extend this toward TRL 6+ (more scenes, measured Jetson power, etc.).
